...
bool CTCPNetworkItem::SendData(WORD wMainCmdID, WORD wSubCmdID, WORD wRountID)
{
	...
	{
		std::string sDataBuffer((char*)pOverLappedSend->m_cbBuffer, pOverLappedSend->m_WSABuffer.len);
		char cbSendData[SOCKET_TCP_BUFFER];

		int encodesize = zl::net::ws::encodeFrame(zl::net::ws::WsFrameType::WS_TEXT_FRAME, sDataBuffer.c_str(), sDataBuffer.size(), cbSendData, SOCKET_TCP_BUFFER);
...
bool CTCPNetworkItem::SendData(VOID * pData, WORD wDataSize, WORD wMainCmdID, WORD wSubCmdID, WORD wRountID)
{
	...
	if (m_connectType == WEBSOCKET)
	{
		...
		int encodesize = zl::net::ws::encodeFrame(zl::net::ws::WsFrameType::WS_TEXT_FRAME, data.c_str(), data.size(), cbSendData, SOCKET_TCP_BUFFER);
...
bool CTCPNetworkItem::OnRecvCompleted(COverLappedRecv * pOverLappedRecv, DWORD dwThancferred)
{
	...
	{
	case CTCPNetworkItem::WINSOCKET: {
	...
	case CTCPNetworkItem::WEBSOCKET: {
...
bool CTCPNetworkItem::HandleWebsocketRecv()
{
	...
	try
	{
		while (m_wRecvSize > 0)
		{
			...
			switch (type)
			{
			case zl::net::ws::WS_INCOMPLETE_TEXT_FRAME:
			case zl::net::ws::WS_INCOMPLETE_BINARY_FRAME:
			case zl::net::ws::WS_INCOMPLETE_FRAME: {
				return true;
			}
			case zl::net::ws::WS_TEXT_FRAME:
			case zl::net::ws::WS_BINARY_FRAME: {
				m_dwRecvPacketCount++;

				TCP_Command Command = ((TCP_Head *)outbuf)->CommandInfo;
				...
				if (Command.wMainCmdID == MDM_KN_COMMAND)
				{
					...
					case SUB_KN_VALIDATE_SOCKET:
						//TCHAR szValidateCode[MAX_PATH] = TEXT("");
						//std::string strVaildCode = zl::net::ws::kWebSocketMagicString;
						//memcpy(szValidateCode, outbuf + sizeof(TCP_Head), sizeof(outbuf) - sizeof(TCP_Head));
						//std::string strInputVaild = T2A(szValidateCode);
						break;
					}
					default:
						throw TEXT("非法命令码");
				...
				{
					// 消息处理
					m_pITCPNetworkItemSink->OnEventSocketRead(Command, (void*)(data.c_str() + sizeof(TCP_Head)), data.size()-sizeof(TCP_Head), this);
				}

				break;
			}
			case zl::net::ws::WS_PING_FRAME: {
				char cbSendData[SOCKET_TCP_BUFFER];
				int encodesize = zl::net::ws::encodeFrame(zl::net::ws::WsFrameType::WS_PONG_FRAME, NULL, 0, cbSendData, SOCKET_TCP_BUFFER);
				SendRawData(cbSendData, encodesize);
				break;
			}
			case zl::net::ws::WS_PONG_FRAME: {
				break;
			}
			case zl::net::ws::WS_CLOSE_FRAME: {
				break;
			}
			default: {
				return false;
...
bool CTCPNetworkItem::CheckIfWinSocket()
{
	BYTE cbBuffer[SOCKET_TCP_BUFFER];
	TCP_Head * pHead = (TCP_Head *)m_cbRecvBuf;
...
bool CTCPNetworkItem::handshake(zl::net::ByteBuffer* byteBuffer)
{
	...
	TCHAR szTips[MAX_PATH] = TEXT("");
	wsprintfW(szTips, L"kSecWebSocketKeyHeader >> %s", key);
	g_TraceServiceManager.TraceString(szTips, TraceLevel_Info);
...
{
	// 寻找发生结构
	COverLappedSend * pOverLappedSend = GetSendOverLapped(len);
		...
		while(TRUE)
		{
			if(m_AutoServicPort != PORT_LOGON && m_AutoServicPort != PORT_CENTER && m_AutoServicPort != PORT_MANAGER)
			{
				if(CreateSocket(m_AutoServicPort))
				{
					...
					wsprintfW(pszServicePort,L"未指定网络端口，将随机分配端口:%d",m_AutoServicPort);
					g_TraceServiceManager.TraceString(pszServicePort, TraceLevel_Info);
					break;
				}
			}
...
bool CTCPNetworkEngine::AttestationAcc()
{
	...
	//else
	//{
	//	g_TraceServiceManager.TraceString(TEXT("组件未授权,请联系客服 QQ:9206752"), TraceLevel_Exception);
	//	m_bValidate = false;
	//	return false;
	//}
}
...
//		wsprintfW(pszString,L"网络绑定错误,启动失败.占用端口：%d",m_AutoServicPort);
//        g_TraceServiceManager.TraceString(pszString,TraceLevel_Exception);
        closesocket(m_hServerSocket);
        m_hServerSocket = INVALID_SOCKET;
        return false;
