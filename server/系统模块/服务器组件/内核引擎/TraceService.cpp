...
bool CTraceServiceControl::LoadMessage(LPCTSTR pszFileName)
{
    ...
    // 打开文件
    CFile File;
    if (File.Open(pszFileName, CFile::modeRead, NULL) == FALSE)
        return false;
...
bool CTraceServiceControl::SaveMessage(LPCTSTR pszFileName)
{
    ...
    // 打开文件
    CFile File;
    if (File.Open(strFileName, CFile::modeWrite | CFile::modeCreate, NULL) == FALSE)
        return false;
...
bool CTraceServiceControl::SetParameter(LONG lMaxLineCount, LONG lReserveLineCount)
{
    ...
    if ((m_lMaxLineCount != 0) && (m_lMaxLineCount <= m_lReserveLineCount))
        return false;
...
LRESULT CTraceServiceControl::OnTraceServiceMessage(WPARAM wParam, LPARAM lParam)
{
    ...
    if (DataHead.wIdentifier != IDT_TRACE_STRING)
        return 0;
